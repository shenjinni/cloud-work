$(function(){
	var subform={
			...
			handButton:function(table){
				...
				table.delegate(".dell", "click", function(){
					self.delRows(this);
				}); 
				
				table.delegate(".pricedell", "click", function(){
					self.delPriceRows(this);
				}); 
				
				table.delegate(".pricedel", "click", function(){
					self.delPriceRow(this);
				});  
			...
			_addRow:function (table){
				var max =window.subNum || 15;
				var self = this;
				var row=table.data('row');
				var tr=$(row).clone();
				var count=table.find("tr[type='subdata']").length;
				if(count>=max){
					return;
				}
				table.append(tr);
				self.renderRow(tr);
				if(window.subTrAutocomplete){
					subTrAutocomplete(tr);
				}
				return tr;
			},
			...
			delPriceRow:function(obj){
				...
				//取删除按钮的table********新增必须存在一行记录校验 *****gd  2017-05-16
				...
				for(var i = 0; i < rows.length; i++){
					for(var j = 0; j < rows[i].cells.length; j++){
						if(rows[i].cells[j].innerText == '删除'){
							count++;
						}
					}
				}
				if(count == 1){
					parent.layer.alert("第一行不允许删除。", {
