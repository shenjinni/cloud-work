...
<html>
<head>
<title>cloud_work_labor管理</title>
	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label">cloud_work_labor管理列表</span>
			</div>
		...
		<div class="panel-body">
		    ...
		    <display:table name="CloudWorkLaborList" id="CloudWorkLaborItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="true"  class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
			  		<input type="checkbox" class="pk" name="id" value="${CloudWorkLaborItem.id}">
				</display:column>
				...
				<display:column  title="发布时间" sortable="true" sortName="createTime">
					<fmt:formatDate value="${CloudWorkLaborItem.createTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				<display:column  title="更新时间" sortable="true" sortName="updateTime">
					<fmt:formatDate value="${CloudWorkLaborItem.updateTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				<display:column property="updateUser" title="修改者" sortable="true" sortName="updateUser"></display:column>
				<display:column title="管理" media="html" style="width:180px">
					<a href="del.ht?id=${CloudWorkLaborItem.id}" class="link del">删除</a>
					<a href="edit.ht?id=${CloudWorkLaborItem.id}" class="link edit">编辑</a>
					<a href="get.ht?id=${CloudWorkLaborItem.id}" class="link detail">明细</a>
					
				</display:column>
			</display:table>
			<hotent:paging tableId="CloudWorkLaborItem"/>
